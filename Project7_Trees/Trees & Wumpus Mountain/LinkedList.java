 ...
 * @modified: March 13, 2014 - Modified for the Wumpus homework, Nodes can only take in mountian caves instead of objects.
 *            The toString method has been changed to fit the Wumpus homework.
 * 
 */
public class LinkedList 
{
     ...
     */
    public void insert(MountainCave data, int index){
        if(index > size){ 
     ...
     * @modified: toString has changed for the Wumpus homework. The method now puts all the caves name into
     *            a string array and prints the names from the tail to the head.
    ...
    public String toString(){
        ...
        String retVal = "";
        String[] listOfPaths = new String[size];//Creates a array of strings
        Node current = head;

        //Puts all caves names into the array
        for(int i = 0; i < size; i++){
            listOfPaths[i] = current.data.getCaveName();
            current = current.next;
        }
        
        //prints out the array from the end to the beginning
        for(int i = size -1; i >= 0; i--){
            retVal += listOfPaths[i] + "\n";
        }
    ...
    private class Node{
        ...
        public Node(MountainCave newItem, Node linkValue){
            data = newItem;
            next = linkValue;
        }
